...
public class TestRobotPaths {
	...
	public void test2by2Path() {
		assertEquals("{[1,1][1,2][2,2]}{[1,1][2,1][2,2]}",
				buildString(new RobotPaths().paths(2,2, (r, c)->false)));
	}
	...
	public void test3by3PathAvoidCentre() {
		assertEquals("{[1,1][1,2][1,3][2,3][3,3]}{[1,1][2,1][3,1][3,2][3,3]}", buildString(new RobotPaths().paths(10,10, (r, c)->(r==2&&c==2))));
	}
	
	private String buildString(final List<String> paths) {
		return paths.stream().map((s)->"{"+s+"}").collect(Collector.of(StringBuilder::new, StringBuilder::append,
				(l,r)->{l.append(r); return l;})).toString();
	}
